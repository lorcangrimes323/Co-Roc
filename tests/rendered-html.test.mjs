...
test("imports, reconstructs and stores post-flight trajectories", async () => {
  ...
  assert.match(map, /id: "measured-glow"/);
  assert.match(map, /id: "measured-ground"/);
  assert.match(map, /id: "measured-3d"/);
  assert.match(map, /trajectoryFitMaxZoom/);
  assert.doesNotMatch(map, /!map\.isStyleLoaded\(\) \|\| !map\.getSource\("terrain-dem"\)/);
  assert.match(map, /pointPrisms\(measuredEventPoints/);
  assert.match(map, /source\.setData\(pointPrisms\(\[activePoint\]/);
