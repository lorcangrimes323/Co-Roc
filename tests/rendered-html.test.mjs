...
test("keeps OpenRocket geometry and live edits traceable", async () => {
  ...
  assert.match(missionControl, /configuration-summary-height/);
  assert.match(missionControl, /Resize vehicle summary/);
  assert.match(missionControl, /aria-orientation="horizontal"/);
...
test("imports, reconstructs and stores post-flight trajectories", async () => {
  ...
  assert.doesNotMatch(map, /map\.easeTo\(\{/);
  ...
  assert.match(map, /"line-width": 1\.25/);
