...
import { buildFlightData, inferFlightMapping, parseFlightTable, simulatedGroundTrack, type FlightChannel, type FlightColumnMapping, type FlightPoint, type FlightSummary } from "../lib/flight-data";
...
type FlightRecord = Omit<FlightSummary, "apogee"> & {
  ...
  importedByName: string; createdAt: string; warnings: string[]; maxAltitude: number;
};
type Trajectory = { points: FlightPoint[]; mapping?: FlightColumnMapping; columns?: string[]; warnings?: string[] };
type Props = {
...
function FlightImportModal({ model, workspaceVersion, theme, onCancel, onComplete, onNotice, mode, headers }: { model: OpenRocketModel | null; workspaceVersion: number | null; theme: "light" | "dark"; onCancel: () => void; onComplete: (record?: FlightRecord, trajectory?: Trajectory) => void; onNotice: (message: string) => void; mode: "demo" | "live"; headers: () => Record<string, string> }) {
  ...
  async function choose(next: File | null) {
    if (!next) return;
    const contents = await next.text();
    const table = parseFlightTable(contents);
    setFile(next); setText(contents); setColumns(table.columns); setMapping(inferFlightMapping(table.columns)); setName(next.name.replace(/\.[^.]+$/, "")); setPreview(null); setError("");
  }
  function rebuild() {
    try { setPreview(buildFlightData(text, { mapping, launchLatitude: site.latitude, launchLongitude: site.longitude, launchAltitude: site.altitude, headingDegrees: site.heading })); setError(""); }
    catch (reason) { setPreview(null); setError(reason instanceof Error ? reason.message : "Preview failed."); }
  }
  async function save() {
    if (!file) { setError("Choose a CSV flight log first."); return; }
    let built = preview;
    if (!built) { try { built = buildFlightData(text, { mapping, launchLatitude: site.latitude, launchLongitude: site.longitude, launchAltitude: site.altitude, headingDegrees: site.heading }); } catch (reason) { setError(reason instanceof Error ? reason.message : "Import failed."); return; } }
    if (mode === "demo") {
      onComplete({ id: `demo-${Date.now()}`, name: name || file.name, flightDate, computer, sourceFileName: file.name, launchSiteName: site.name, launchLatitude: site.latitude, launchLongitude: site.longitude, launchAltitude: site.altitude, headingDegrees: site.heading, orkVersion: workspaceVersion, importedByName: "Demo user", createdAt: new Date().toISOString(), warnings: built.warnings, sampleCount: built.summary.sampleCount, duration: built.summary.duration, maxAltitude: built.summary.apogee, maxVelocity: built.summary.maxVelocity, maxAcceleration: built.summary.maxAcceleration, maxDistance: built.summary.maxDistance, landingDistance: built.summary.landingDistance, hasGps: built.summary.hasGps }, { points: built.points, mapping: built.mapping, columns: built.columns, warnings: built.warnings });
    ...
    try {
      ...
      onComplete(payload.flights?.find((item) => item.id === payload.selectedId), { points: built.points, mapping: built.mapping, columns: built.columns, warnings: built.warnings });
  ...
  return <div className="flight-modal-backdrop"><section className="flight-import-modal" role="dialog" aria-modal="true" aria-labelledby="flight-import-title">
    <header><div><span>POST-FLIGHT DATA</span><h2 id="flight-import-title">Import a CATS Vega flight</h2><p>Map the logger channels, establish the launch datum and inspect the reconstructed trajectory before it becomes part of the record.</p></div><button type="button" onClick={onCancel} aria-label="Close import">&times;</button></header>
    <div className="flight-import-grid"><div className="flight-import-form">
      <section><h3>1 · Source log</h3><button className="flight-file-picker" type="button" onClick={() => fileRef.current?.click()}><strong>{file ? file.name : "Choose CSV or text log"}</strong><span>{file ? `${(file.size / 1024).toFixed(1)} KB · ${columns.length} columns detected` : "CATS Vega and generic delimited telemetry are supported"}</span></button><input ref={fileRef} hidden type="file" accept=".csv,.txt,.tsv,text/csv,text/plain" onChange={(event) => void choose(event.target.files?.[0] ?? null)} />
      ...
      <section><h3>2 · Channel mapping</h3><div className="flight-mapping-grid">{channelDetails.map(({ key, label, optional }) => <label key={key}>{label}<small>{optional ? "OPTIONAL" : "REQUIRED"}</small><select value={mapping[key] ?? ""} onChange={(event) => setMapping((current) => ({ ...current, [key]: event.target.value || undefined }))}><option value="">Not mapped</option>{columns.map((column) => <option key={column}>{column}</option>)}</select></label>)}</div></section>
      <section><h3>3 · Launch datum</h3><div className="flight-basic-grid"><label>Site name<input value={site.name} onChange={(event) => setSite((value) => ({ ...value, name: event.target.value }))} placeholder="Approved range" /></label><label>Latitude<input type="number" step="0.000001" value={site.latitude} onChange={(event) => setSite((value) => ({ ...value, latitude: Number(event.target.value) }))} /></label><label>Longitude<input type="number" step="0.000001" value={site.longitude} onChange={(event) => setSite((value) => ({ ...value, longitude: Number(event.target.value) }))} /></label><label>Altitude MSL<input type="number" value={site.altitude} onChange={(event) => setSite((value) => ({ ...value, altitude: Number(event.target.value) }))} /></label><label>Launch heading<input type="number" min="0" max="360" value={site.heading} onChange={(event) => setSite((value) => ({ ...value, heading: Number(event.target.value) }))} /></label></div><FlightPathMap compact theme={theme} launchSite={{ latitude: site.latitude, longitude: site.longitude, altitude: site.altitude }} measured={preview?.points ?? []} onLaunchSiteChange={({ latitude, longitude }) => setSite((value) => ({ ...value, latitude, longitude }))} /></section>
    </div><aside className="flight-import-preview"><span>RECONSTRUCTION CHECK</span>{preview ? <><Metric label="Valid samples" value={preview.summary.sampleCount.toLocaleString()} /><Metric label="Apogee" value={Math.round(preview.summary.apogee).toLocaleString()} unit="m MSL" /><Metric label="Ground track" value={Math.round(preview.summary.maxDistance).toLocaleString()} unit="m" /><div className="flight-import-warnings">{preview.warnings.map((warning) => <p key={warning}>{warning}</p>)}{!preview.warnings.length && <p className="flight-valid">GNSS trajectory detected and ready to store.</p>}</div></> : <p>Choose a log, confirm time and altitude, then generate the preview.</p>}<button type="button" className="button button-secondary" disabled={!file} onClick={rebuild}>Generate preview</button></aside></div>
...
export function PostFlightWorkspace({ model, mode, workspaceVersion, headers, canImport, canDelete, theme, onNotice }: Props) {
  ...
  const [trajectories, setTrajectories] = useState<Record<string, Trajectory>>(mode === "demo" ? { [demoRecord.id]: { points: demo.points } } : {});
  const [importOpen, setImportOpen] = useState(false); const [loading, setLoading] = useState(mode === "live"); const [playing, setPlaying] = useState(false); const [timeIndex, setTimeIndex] = useState(0); const [showSimulation, setShowSimulation] = useState(true);
  ...
  useEffect(() => {
    ...
    void (async () => { try { const response = await fetch(`/api/flights?flightId=${encodeURIComponent(selected.id)}`, { headers: headers(), cache: "no-store" }); if (!response.ok) throw new Error(); const data = await response.json() as Trajectory; setTrajectories((values) => ({ ...values, [selected.id]: data })); } catch { onNotice("The selected flight trajectory could not be loaded"); } })();
  }, [selected?.id, mode]);
  useEffect(() => { setTimeIndex(0); setPlaying(false); }, [selectedId]);
  useEffect(() => { if (!playing || !trajectory?.points.length) return; const timer = window.setInterval(() => setTimeIndex((index) => index >= trajectory.points.length - 1 ? 0 : index + 1), 35); return () => window.clearInterval(timer); }, [playing, trajectory?.points.length]);
  ...
  return <section className="workspace-module postflight-module"><header className="postflight-heading"><div><span>FLIGHT TEST DATA</span><h1>Post-flight visualisation</h1><p>Reconstruct measured flights on 3D terrain, compare them with the predicted OpenRocket path and retain the evidence against the working configuration.</p></div><button className="button button-primary" type="button" disabled={!canImport} onClick={() => setImportOpen(true)}>＋ Import flight data</button></header>
    <div className="postflight-layout"><aside className="flight-record-list"><header><span>FLIGHT RECORDS</span><strong>{flights.length}</strong></header>{loading && <p>Loading flight records…</p>}{flights.map((flight) => <button key={flight.id} type="button" className={flight.id === selected?.id ? "active" : ""} onClick={() => setSelectedId(flight.id)}><span>{flight.flightDate ? new Date(`${flight.flightDate}T12:00:00`).toLocaleDateString(undefined, { day: "2-digit", month: "short", year: "numeric" }) : "UNDATED"}</span><strong>{flight.name}</strong><small>{flight.computer} · {flight.sampleCount.toLocaleString()} samples</small><em>{flight.hasGps ? "GNSS" : "RECONSTRUCTED"}</em></button>)}{!loading && !flights.length && <div className="flight-empty"><strong>No flight data yet</strong><span>Import a CATS Vega or generic CSV log to start the post-flight record.</span></div>}</aside>
      <main className="flight-visualiser">{selected ? <><div className="flight-visualiser-bar"><div><span>MEASURED FLIGHT</span><h2>{selected.name}</h2><p>{selected.launchSiteName || `${selected.launchLatitude.toFixed(5)}, ${selected.launchLongitude.toFixed(5)}`} · linked to W{selected.orkVersion ?? "—"}</p></div><label><input type="checkbox" checked={showSimulation} disabled={!simulated.length} onChange={(event) => setShowSimulation(event.target.checked)} /> Simulation overlay</label></div>
        <FlightPathMap theme={theme} measured={trajectory?.points ?? []} simulated={showSimulation ? simulated : []} currentIndex={timeIndex} launchSite={{ latitude: selected.launchLatitude, longitude: selected.launchLongitude, altitude: selected.launchAltitude }} />
        <div className="flight-playback"><button type="button" disabled={!trajectory?.points.length} onClick={() => setPlaying((value) => !value)}>{playing ? "Pause" : "Play flight"}</button><span>{active ? `${active.time.toFixed(1)} s` : "—"}</span><input aria-label="Flight playback position" type="range" min="0" max={Math.max(0, (trajectory?.points.length ?? 1) - 1)} value={timeIndex} onChange={(event) => { setPlaying(false); setTimeIndex(Number(event.target.value)); }} /><span>{selected.duration.toFixed(1)} s</span></div></> : <div className="postflight-empty-map"><strong>Select or import a flight</strong><span>The 3D trajectory and flight channels will appear here.</span></div>}</main>
      <aside className="flight-inspector">{selected && <><header><span>FLIGHT SUMMARY</span><h2>{selected.name}</h2><p>{selected.sourceFileName}</p></header><div className="flight-metric-grid"><Metric label="Apogee" value={Math.round(selected.maxAltitude).toLocaleString()} unit="m" /><Metric label="Max speed" value={selected.maxVelocity === null ? "—" : selected.maxVelocity.toFixed(1)} unit="m/s" /><Metric label="Max accel." value={selected.maxAcceleration === null ? "—" : selected.maxAcceleration.toFixed(1)} unit="m/s²" /><Metric label="Ground range" value={Math.round(selected.maxDistance).toLocaleString()} unit="m" /></div><section className="flight-current-state"><span>CURSOR</span><dl><div><dt>Time</dt><dd>{active?.time.toFixed(2) ?? "—"} s</dd></div><div><dt>Altitude</dt><dd>{active ? Math.round(active.altitude).toLocaleString() : "—"} m</dd></div><div><dt>Velocity</dt><dd>{active?.velocity?.toFixed(1) ?? "—"} m/s</dd></div><div><dt>Position</dt><dd>{active ? `${active.latitude.toFixed(5)}, ${active.longitude.toFixed(5)}` : "—"}</dd></div></dl></section><div className="flight-charts"><ChannelChart points={trajectory?.points ?? []} channel="altitude" label="Altitude · m MSL" colour="#d4253b" /><ChannelChart points={trajectory?.points ?? []} channel="velocity" label="Velocity · m/s" colour="#157f54" /><ChannelChart points={trajectory?.points ?? []} channel="acceleration" label="Acceleration · m/s²" colour="#b1721b" /></div><section className="flight-provenance"><span>TRACEABILITY</span><p><strong>Imported by</strong>{selected.importedByName}</p><p><strong>Working copy</strong>W{selected.orkVersion ?? "—"}</p><p><strong>Trajectory</strong>{selected.hasGps ? "Measured GNSS" : "Derived ground track"}</p></section>{canDelete && mode === "live" && <button className="flight-delete" type="button" onClick={() => void removeFlight()}>Remove flight record</button>}</>}</aside></div>
    {importOpen && <FlightImportModal model={model} workspaceVersion={workspaceVersion} theme={theme} onCancel={() => setImportOpen(false)} onComplete={complete} onNotice={onNotice} mode={mode} headers={headers} />}
  </section>;
}
