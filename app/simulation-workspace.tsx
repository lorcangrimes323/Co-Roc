...
export function SimulationWorkspace({ model, mode, workspaceVersion, headers, canRun, onNotice, onModelChange }: {
  ...
  return <section className="workspace-module simulation-module">
    ...
    <div className="simulation-content">
      ...
      {configured && mode === "demo" && <div className="solver-banner"><strong>Local solver connected.</strong><span>Demo calculations are real but are not written to team run history.</span></div>}
    ...
    <aside className="run-history"><header><span className="eyebrow">TRACEABLE RUNS</span><h2>Run history</h2></header>{runs.map((run) => <article key={run.id}><strong>{run.simulationName}</strong><span>ORK V{run.orkVersion} · Core {run.engineVersion}</span><small>{run.runByName} · {new Date(run.createdAt).toLocaleString()}</small></article>)}{!runs.length && <div className="module-empty">{mode === "demo" ? "Demo runs are not stored." : "No server calculations recorded yet."}</div>}<footer>{calculatedAt && `Latest calculation ${new Date(calculatedAt).toLocaleString()} · Core ${engineVersion}`}</footer></aside>
